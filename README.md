...
4. Set `YOUR_APP_URL` (your ngrok or other platform URL) and `YOUR_APP_URL/callback` to your app settings in [partner dashboard](https://partners.shopify.com/) (if you add `?external=true` parameter to `YOUR_APP_URL`, the app UX turns into a service connector which tries to connect Shopify stores and their users). 
