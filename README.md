...
package.json ... app.js used pnpm libaries and scripts for building and running this app.
...
frontend/ ... React code base to be built by Vite for app admin UI. All built code runs at the client = browser as minized JS + CSS.
  ...
  ./package.json ... React code used pnpm libraries and scripts for building React code with Vite.
...
# How to update
- For server side update (`app.js` or `views` in Koa), run the start command (`pnpm run start`) again works. Some cloud services like Render enables it with `git commit & git push`.
- For fronend update (`frontend` in React), run the build command (`pnpm run build`) again. If you change the value of `SHOPIFY_API_KEY`, you need to build again. Some cloud services like Render enables it with `git commit & git push`.
