    ...
    return (
        <form method="post" encType="multipart/form-data" onSubmit={uploadFile}>
            ...
            {selectedOperation === PRODUCT_CREATE ? (
                <p>
                    Product creation format: each line contains <b>product</b> (<s-link href="https://shopify.dev/docs/api/admin-graphql/unstable/input-objects/ProductCreateInput" target="_blank">ProductCreateInput</s-link>) and optional <b>media</b> (<s-link href="https://shopify.dev/docs/api/admin-graphql/unstable/input-objects/CreateMediaInput" target="_blank">CreateMediaInput</s-link>). Put each public image URL in <b>media[].originalSource</b>. The bundled sample provides one ordered media entry for every variant in the matching variant file. You can specify <b>product.handle</b> before creation and use it to match the Result data later. You can't preassign <b>product.id</b> or a media GID; Shopify generates and returns them after creation.
                </p>
            ) : (
                <>
                    <p>
                        Variant creation format: each line contains one or more <b>variants</b> (<s-link href="https://shopify.dev/docs/api/admin-graphql/unstable/input-objects/ProductVariantsBulkInput" target="_blank">ProductVariantsBulkInput</s-link>) for one product. Each variant supplies one value for every option on that product, such as <b>Size: Small / Color: Red</b>. Shopify requires <b>productId</b> for <b>productVariantsBulkCreate</b>. This sample accepts <b>productHandle</b> and replaces it with the returned product GID. When <b>assignMediaByPosition</b> is true, it also assigns the returned media IDs to variants in array order before staging.
                    </p>
                    <br />
                    <s-drop-zone label="Product creation Result data JSONL file" name="productResultFile"></s-drop-zone>
                    <p>
                        When the variant file uses <b>productHandle</b> or <b>assignMediaByPosition</b>, download the completed product creation operation's <b>Result data</b> in step 3 and upload it here. The app matches each handle to its Shopify product ID and ordered media IDs without individual product lookup requests. Alternatively, when every record already contains <b>productId</b> and every variant that needs media contains <b>mediaId</b>, the Result data file isn't required.
                    </p>
                </>
            )}
            <p>
